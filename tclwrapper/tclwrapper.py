...
class TCLWrapperError(TCLWrapperException):
    ...
    def __str__(self):
        msg = (
            'TCLWrapperError raised while executing the command "%s"\n error_message: "%s"'
...
class TCLWrapper:
    ...
    def eval(self, command, to_list=False):
        ...
        self._process.stdin.write(
            b'puts -nonewline stdout "' + stdout_start_key + b'"\n'
        )
        self._process.stdin.write(
            b'puts -nonewline stderr "' + stderr_start_key + b'"\n'
        )
        self._process.stdin.write(bytearray(main_tcl_code, "utf-8"))
        self._process.stdin.write(
            b'puts -nonewline stdout "' + stdout_done_key + b'"\n'
        )
        self._process.stdin.write(
            b'puts -nonewline stderr "' + stderr_done_key + b'"\n'
        )
        ...
        try:
            while fetching_stdout or fetching_stderr:
                ...
                if return_code is not None:
                    raise TCLWrapperInstanceError(
                        "tcl process finished unexpectedly with return code %d"
                        % return_code
                    )
                if fetching_stdout:
                    try:
            ...
            print(
                "KeyboardInterrupt raised while trying to read from stdout and stderr in TCLWrapper('%s')"
                % self.tcl_exe
            )
        ...
        stdout = stdout[
            stdout_start_key_loc + len(stdout_start_key) : -len(stdout_done_key)
        ].decode("utf-8")
        stderr = stderr[
            stderr_start_key_loc + len(stderr_start_key) : -len(stderr_done_key)
        ].decode("utf-8")
        ...
        if len(stderr_split) == 3:
            ...
            if cmd_stderr:
                warnings.warn(
                    'tcl command "%s" generated stderr message %s'
                    % (command, repr(cmd_stderr)),
                    stacklevel=2,
                )
            if unexpected_stderr:
                # This shouldn't happen
                raise RuntimeError(
                    'tcl command "%s" produced unexpected stderr message %s'
            ...
            raise RuntimeError(
                'tcl command "%s" produced stderr with an unexpected number of stderr delimiter keys. Full stderr message: %s'
                % (command, repr(stderr))
            )
        else:
            if stderr:
