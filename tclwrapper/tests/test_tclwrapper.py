...
class TestTCLWrapper(unittest.TestCase):
    ...
    @unittest.skipIf(
        shutil.which("wish") is None or os.getenv("XDG_SESSION_TYPE") in ["tty", None],
        "test requires wish to be in the path",
    )
    def test_tclwrapper_wish(self):
        ...
        with tclwrapper.TCLWrapper() as tcl:
            try:
                ...
                self.fail(
                    '"wm withdraw ." should fail with a TCLWrapperError in normal tclsh'
                )
    ...
    @unittest.skipIf(
        shutil.which("gtkwave") is None
        or os.getenv("XDG_SESSION_TYPE") in ["tty", None],
        "test requires gtkwave to be in the path",
    )
